...
st.markdown(
    "<div class='main-title'>⚽ 2026 World Cup AI Predictor</div>",
    unsafe_allow_html=True,
)
st.markdown(
    "<div class='sub-title'>推論エージェント討論 ＆ モンテカルロ・トーナメントシミュレーター</div>",
    unsafe_allow_html=True,
)
...
with tab1:
    ...
    with col1:
        team_a = st.selectbox(
            "Team A (Home)",
            team_names,
            index=team_names.index("Japan") if "Japan" in team_names else 0,
        )
    with col2:
        team_b = st.selectbox(
            "Team B (Away)",
            team_names,
            index=team_names.index("Brazil") if "Brazil" in team_names else 1,
        )

    # スタジアム選択を追加
    stadium_options = {
        "MetLife Stadium (New York/New Jersey) [標準環境]": (
            "MetLife Stadium (New York/New Jersey)",
            "standard",
        ),
        "Estadio Azteca (Mexico City) [標高 2,200m 高地]": (
            "Estadio Azteca (Mexico City)",
            "altitude",
        ),
        "BC Place (Vancouver) [高速人工芝]": ("BC Place (Vancouver)", "turf"),
        "Hard Rock Stadium (Miami) [酷暑多湿]": ("Hard Rock Stadium (Miami)", "heat"),
        "NRG Stadium (Houston) [空調管理ドーム]": ("NRG Stadium (Houston)", "standard"),
    }
    selected_stadium_label = st.selectbox(
        "🏟️ 開催スタジアム（環境条件）を選択してください", list(stadium_options.keys())
    )
    stadium_name, stadium_env = stadium_options[selected_stadium_label]

    # 休養日数の選択を追加
    rest_options = {"十分な休養 (中4日以上)": 4, "中3日": 3, "中2日": 2}
    st.markdown("#### ⏳ 試合日程（休養日数）の設定")
    col_rest_a, col_rest_b = st.columns(2)
    with col_rest_a:
        rest_days_a_label = st.selectbox(
            f"{team_a} の休養期間", list(rest_options.keys()), index=0
        )
        rest_days_a = rest_options[rest_days_a_label]
    with col_rest_b:
        rest_days_b_label = st.selectbox(
            f"{team_b} の休養期間", list(rest_options.keys()), index=0
        )
        rest_days_b = rest_options[rest_days_b_label]
    ...
    if st.button("AI予想を実行する", type="primary"):
        if team_a == team_b:
            ...
            with st.spinner("番記者エージェントが過去のデータを元に討論中..."):
                ...
                # 重要度別の加重平均得点力を動的に算出
                team_a_weighted_goals = get_weighted_avg_goals(
                    team_a, fallback_value=float(team_a_data["avg_goals"])
                )
                team_b_weighted_goals = get_weighted_avg_goals(
                    team_b, fallback_value=float(team_b_data["avg_goals"])
                )
                ...
                initial_state = {
                    ...
                    "team_a_rest_days": rest_days_a,
                    "team_b_rest_days": rest_days_b,
                ...
                with res_col2:
                    ...
                    with st.expander("🔍 計算パラメーター（内訳）"):
                        st.write(
                            f"**{team_a}** Base Elo: {result['team_a_base_elo']} | 戦術: {result['team_a_tactics'].capitalize()} | PKレート: {result.get('team_a_pk_rating', 3)} | 休養: 中{result.get('team_a_rest_days', 4)}日 | 市場価値: {result['team_a_market_value']:.0f}M€ | 加重平均得点: {team_a_weighted_goals:.2f}"
                        )
                        st.write(
                            f"**{team_b}** Base Elo: {result['team_b_base_elo']} | 戦術: {result['team_b_tactics'].capitalize()} | PKレート: {result.get('team_b_pk_rating', 3)} | 休養: 中{result.get('team_b_rest_days', 4)}日 | 市場価値: {result['team_b_market_value']:.0f}M€ | 加重平均得点: {team_b_weighted_goals:.2f}"
                        )
                        st.write(
                            f"**{team_a}** 最終期待得点: {result['expected_goals_a']:.2f} (討論補正: {result['team_a_modifier']:.2f})"
                        )
                        st.write(
                            f"**{team_b}** 最終期待得点: {result['expected_goals_b']:.2f} (討論補正: {result['team_b_modifier']:.2f})"
                        )
                        ...
                        if h2h_data:
                            ...
                            for m in h2h_data[:5]:
                                st.write(
                                    f"- {m['date']}: {m['home_team']} {m['home_score']} - {m['away_score']} {m['away_team']} ({m['tournament']})"
                                )
                        else:
...
with tab2:
    ...
    st.markdown(
        "決定した実際の決勝トーナメント（ラウンド32）の組み合わせから、大会全体の行方を1,000回シミュレートして各国の勝ち上がり確率を求めます。"
    )

    # データベースが空の場合は警告を表示
    if not all_teams:
        st.warning(
            "⚠️ データベースに48チームが登録されていません。先に `update_db.py` を実行するか、Supabaseで `update_schema.sql` を実行してください。"
        )

    sim_button = st.button(
        "シミュレーションを実行する (1,000回)", type="primary", disabled=not all_teams
    )
    ...
    if sim_button and all_teams:
        with st.spinner("ラウンド32から決勝戦までの1,000大会を高速シミュレート中..."):
            prob_results = simulate_tournament_monte_carlo(
                all_teams, num_simulations=1000
            )

            # 結果をDataFrameに変換して集計 (ラウンド32進出していない脱落チームは除外)
            rows = []
    ...
    if st.session_state.sim_results is not None:
        ...
        # 優勝確率順にソート
        df_winner_sorted = df_res.sort_values(
            by="優勝確率 🏆", ascending=False
        ...
        # Altairを使用して降順ソートされたカスタム棒グラフを描画
        ...
        chart = (
            ...
            .encode(
                x=alt.X(
                    "国名:N", sort="-y", title="国名", axis=alt.Axis(labelAngle=-30)
                ),
                y=alt.Y("優勝確率 🏆:Q", title="優勝確率", axis=alt.Axis(format=".0%")),
                tooltip=["国名", alt.Tooltip("優勝確率 🏆", format=".1%")],
            )
        ...
        search_team = st.selectbox(
            "詳細を見たいチームを選択してください",
            sorted(df_res["国名"].unique()),
            key="search_team_selectbox",
        )
