...
class AgentAnalysisOutput(BaseModel):
    agent_analysis: str = Field(description="両チームの討論を踏まえた、最終的な戦術分析・解説サマリー")
    team_a_modifier: float = Field(description="Team Aの得点力に対する補正係数（0.8〜1.2）")
    team_b_modifier: float = Field(description="Team Bの得点力に対する補正係数（0.8〜1.2）")
...
def get_stadium_modifier(team_name: str, team_tactics: str, stadium_env: str, host_country: str) -> float:
    """
    ...
    if stadium_env == "altitude":
        ...
        accustomed_teams = {"Brazil", "Argentina", "Uruguay", "Colombia", "Paraguay", "Ecuador", "Mexico"}
        ...
        heat_sensitive = {
            "Germany", "Spain", "France", "England", "Portugal", "Netherlands", "Belgium", "Croatia", 
            "Switzerland", "Austria", "Czechia", "Scotland", "Norway", "Sweden", "Bosnia and Herzegovina", 
            "Türkiye", "Japan", "South Korea"
        }
...
def debate_agent_a_node(state: WorldCupState) -> dict:
    ...
    if not api_key:
        return {
            "debate_history": [{"agent": f"{team_a} 担当記者", "content": f"{team_a}は今回の対戦において優位に立っています。"}]
        }
    ...
    try:
        ...
        return {
            "debate_history": [{"agent": f"{team_a} 担当記者", "content": result.content}]
        }
    except Exception as e:
        print(f"Error in debate_agent_a: {e}")
        return {
            "debate_history": [{"agent": f"{team_a} 担当記者", "content": f"エラーのため主張を生成できませんでした: {e}"}]
        }
...
def debate_agent_b_node(state: WorldCupState) -> dict:
    ...
    if not api_key:
        return {
            "debate_history": debate_history + [{"agent": f"{team_b} 担当記者", "content": f"{team_b}こそがこの試合を支配するでしょう。"}]
        }
    ...
    try:
        ...
        return {
            "debate_history": debate_history + [{"agent": f"{team_b} 担当記者", "content": result.content}]
        }
    except Exception as e:
        print(f"Error in debate_agent_b: {e}")
        return {
            "debate_history": debate_history + [{"agent": f"{team_b} 担当記者", "content": f"エラーのため主張を生成できませんでした: {e}"}]
        }
...
def debate_rebuttal_a_node(state: WorldCupState) -> dict:
    ...
    team_b = state["team_b"]
    debate_history = state.get("debate_history", [])
    opponent_arg = debate_history[-1]["content"] if len(debate_history) >= 1 else "特になし"
    my_first_arg = debate_history[-2]["content"] if len(debate_history) >= 2 else "特になし"
    
    api_key = os.environ.get("GOOGLE_API_KEY")
    if not api_key:
        return {
            "debate_history": debate_history + [{"agent": f"{team_a} 担当記者 (再反論)", "content": "私たちの分析が正しいことは試合で証明されます。"}]
        }
    ...
    try:
        ...
        return {
            "debate_history": debate_history + [{"agent": f"{team_a} 担当記者 (再反論)", "content": result.content}]
        }
    except Exception as e:
        return {
            "debate_history": debate_history + [{"agent": f"{team_a} 担当記者 (再反論)", "content": f"エラーが発生しました: {e}"}]
        }
...
def debate_rebuttal_b_node(state: WorldCupState) -> dict:
    ...
    if not api_key:
        return {
            "debate_history": debate_history + [{"agent": f"{team_b} 担当記者 (最後)", "content": "私たちの勝利は揺るぎません。"}]
        }
    ...
    try:
        ...
        return {
            "debate_history": debate_history + [{"agent": f"{team_b} 担当記者 (最後)", "content": result.content}]
        }
    except Exception as e:
        return {
            "debate_history": debate_history + [{"agent": f"{team_b} 担当記者 (最後)", "content": f"エラーが発生しました: {e}"}]
        }
...
def calc_node(state: WorldCupState) -> dict:
    ...
    # 2. 戦術相性による補正
    tactics_mod_a, tactics_mod_b = get_tactical_modifier(state["team_a_tactics"], state["team_b_tactics"])
    ...
    if "Azteca" in stadium_name:
        ...
        host_country = "Canada"
    elif "Hard Rock" in stadium_name or "MetLife" in stadium_name:
        host_country = "United States"
        
    stadium_mod_a = get_stadium_modifier(team_a, state["team_a_tactics"], stadium_env, host_country)
    stadium_mod_b = get_stadium_modifier(team_b, state["team_b_tactics"], stadium_env, host_country)
    
    # 期待得点の最終算出
    expected_goals_a = state["team_a_avg_goals"] * state.get("team_a_modifier", 1.0) * mv_ratio * tactics_mod_a * stadium_mod_a
    expected_goals_b = state["team_b_avg_goals"] * state.get("team_b_modifier", 1.0) / mv_ratio * tactics_mod_b * stadium_mod_b
    ...
    for i in range(max_goals + 1):
        for j in range(max_goals + 1):
            prob = stats.poisson.pmf(i, expected_goals_a) * stats.poisson.pmf(j, expected_goals_b)
            if i > j:
                prob_a_win += prob
            elif i < j:
                prob_b_win += prob
...
def summary_node(state: WorldCupState) -> dict:
    ...
    summary = f"### 📊 最終勝敗予想結果\n\n"
    summary += f"| チーム | 勝利確率 | 期待得点（補正後） | 戦術スタイル | 市場価値 |\n"
    summary += f"| :--- | :---: | :---: | :---: | :---: |\n"
...
def simulate_match(team_a: dict, team_b: dict, is_knockout: bool = False, stadium_env: str = "standard", host_country: str = "") -> tuple:
    """
    ...
    # 2. スタジアム環境
    stadium_mod_a = get_stadium_modifier(team_a["name"], style_a, stadium_env, host_country)
    stadium_mod_b = get_stadium_modifier(team_b["name"], style_b, stadium_env, host_country)
    
    # Elo差(1.08倍スケールに変更して安定化)と市場価値比、戦術、スタジアムによる得点力の補正
    expected_goals_a = max(0.1, team_a["avg_goals"] * (1.08 ** (diff / 100)) * mv_ratio * tactics_mod_a * stadium_mod_a)
    expected_goals_b = max(0.1, team_b["avg_goals"] * (1.08 ** (-diff / 100)) / mv_ratio * tactics_mod_b * stadium_mod_b)
    ...
    if score_a > score_b:
        ...
        # 引き分け
        if is_knockout:
            winner = team_a["name"] if random.random() < 0.5 else team_b["name"]
            return score_a, score_b, winner
        return score_a, score_b, None

def simulate_group_stage(teams: list) -> dict:
    """
    ...
    for g_name, g_teams in groups.items():
        ...
        standings = {t["name"]: {"team": t, "points": 0, "gd": 0, "gf": 0} for t in g_teams}
        group_matches = []
        ...
        for i in range(4):
            for j in range(i+1, 4):
                t_a = g_teams[i]
                t_b = g_teams[j]
                goals_a, goals_b, winner = simulate_match(t_a, t_b, is_knockout=False, stadium_env=stadium_env, host_country=host_country)
        ...
        while i < 4:
            ...
            j = i + 1
            while j < 4 and pre_sorted[j]["points"] == pre_sorted[i]["points"] and pre_sorted[j]["gd"] == pre_sorted[i]["gd"] and pre_sorted[j]["gf"] == pre_sorted[i]["gf"]:
            ...
            if len(tied_subset) == 2:
                ...
                h2h_match = next((m for m in group_matches if (m["home"] == t1 and m["away"] == t2) or (m["home"] == t2 and m["away"] == t1)), None)
                if h2h_match:
                    goals_1 = h2h_match["home_goals"] if h2h_match["home"] == t1 else h2h_match["away_goals"]
                    goals_2 = h2h_match["away_goals"] if h2h_match["home"] == t1 else h2h_match["home_goals"]
                    if goals_1 < goals_2:
                ...
                mini_standings = {name: {"team_name": name, "points": 0, "gd": 0, "gf": 0} for name in mini_names}
...
def simulate_tournament_monte_carlo(teams: list, num_simulations: int = 1000) -> dict:
    ...
    for _ in range(num_simulations):
        # 1. グループステージ
        res = simulate_group_stage(teams)
        qualified = res["all_qualified"] # 進出32チーム
        
        for t in qualified:
            stats_data[t["name"]]["r32"] += 1
            
        # 2. ラウンド32
        sorted_qualified = sorted(qualified, key=lambda x: x["base_elo"], reverse=True)
        r32_matches = []
        for i in range(16):
            r32_matches.append((sorted_qualified[i], sorted_qualified[31-i]))
            
        # ラウンド32の実行
        r16_teams = []
        for t_a, t_b in r32_matches:
            stadium_name, env, host = get_random_stadium_info()
            _, _, winner = simulate_match(t_a, t_b, is_knockout=True, stadium_env=env, host_country=host)
            winner_team = t_a if winner == t_a["name"] else t_b
            r16_teams.append(winner_team)
            stats_data[winner_team["name"]]["r16"] += 1
        ...
        for i in range(0, 16, 2):
            ...
            _, _, winner = simulate_match(t_a, t_b, is_knockout=True, stadium_env=env, host_country=host)
            winner_team = t_a if winner == t_a["name"] else t_b
            qf_teams.append(winner_team)
            stats_data[winner_team["name"]]["qf"] += 1
        ...
        for i in range(0, 8, 2):
            ...
            _, _, winner = simulate_match(t_a, t_b, is_knockout=True, stadium_env=env, host_country=host)
            winner_team = t_a if winner == t_a["name"] else t_b
            sf_teams.append(winner_team)
            stats_data[winner_team["name"]]["sf"] += 1
        ...
        for i in range(0, 4, 2):
            ...
            _, _, winner = simulate_match(t_a, t_b, is_knockout=True, stadium_env=env, host_country=host)
            winner_team = t_a if winner == t_a["name"] else t_b
            final_teams.append(winner_team)
            stats_data[winner_team["name"]]["final"] += 1
        ...
        stadium_name, env, host = get_random_stadium_info()
        _, _, winner = simulate_match(t_a, t_b, is_knockout=True, stadium_env=env, host_country=host)
        stats_data[winner]["winner"] += 1
        
    prob_results = {}
    for name, counts in stats_data.items():
        prob_results[name] = {stage: count / num_simulations for stage, count in counts.items()}
